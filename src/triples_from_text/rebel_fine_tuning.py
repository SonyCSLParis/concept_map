...
BATCH_SIZE = 4
SEED = 1
SAVE_PATH = 'src/triples_from_text/finetuned_rebel.pth'
...
if __name__ == "__main__":
    ...
    DF_TRAIN = pd.read_csv('src/triples_from_text/cm_biology_train.csv')
    DF_VAL = pd.read_csv('src/triples_from_text/cm_biology_eval.csv')
    #del data
    ...
    f = open("src/triples_from_text/finetuning_report.txt", "w+", encoding='utf-8')
    ...
    test_data = pd.read_csv('src/triples_from_text/cm_biology_test.csv')
    MODEL = torch.load('src/triples_from_text/finetuned_rebel.pth')
    ...
    texts, results = make_predictions(texts=test_data.context.to_list(), path_to_model='src/triples_from_text/finetuned_rebel.pth')
    pd.DataFrame({"context": texts, "triplets": results}).to_csv("src/triples_from_text/cm_biology_test_predicted.csv")
