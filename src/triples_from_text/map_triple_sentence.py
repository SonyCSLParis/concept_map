...
if __name__ == '__main__':
    ...
    for topic in sorted(os.listdir("src/triples_from_text/Corpora/Biology/")):
        print(topic)
        
        try:
            file_n = os.listdir(f"src/triples_from_text/Corpora/Biology/{topic}/gold/")[0]
            triples = pd.read_csv(f"src/triples_from_text/Corpora/Biology/{topic}/gold/{file_n}", header=None)
            file_n = os.listdir(f"src/triples_from_text/Corpora/Biology/{topic}/text/")[0]
            text = open(f"src/triples_from_text/Corpora/Biology/{topic}/text/{file_n}", 'r').read()
            doc = NLP(text)
    ...
    pd.DataFrame({"context": CONTEXT, "triplets": TRIPLETS}).to_csv("src/triples_from_text/cm_biology.csv")
    T_END = datetime.now()
