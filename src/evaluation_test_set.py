    ...
    for root, dirs, files in os.walk(input_folder_path):
        for file_name in files:
            if file_name.endswith(".txt"):
                ...
                last_folder = os.path.basename(root_folder_root)

                gold_file_name = f"{file_number}.txt"

                input_file_path = os.path.join(input_folder_path, last_folder, file_number + "-fine-tuned-relations-transformed.txt")
                gold_file_path = os.path.join(gold_folder_path, last_folder, gold_file_name)
                ...
                with open(input_file_path, "r") as input_file, open(gold_file_path, "r") as gold_file:
                        ...
                        cluster_key = last_folder  # Use the cluster as the key for grouping metrics

                        if cluster_key not in metrics:
                            metrics[cluster_key] = {
                                'METEOR': [],
                                'ROUGE-1': [],
                                'ROUGE-2': [],
                        ...
                        metrics[cluster_key]['METEOR'].append(meteor)
                        metrics[cluster_key]['ROUGE-1'].append(rouge1)
                        metrics[cluster_key]['ROUGE-2'].append(rouge2)
                        metrics[cluster_key]['ROUGE-L'].append(rougeL)

    output_file_path = os.path.join(output_folder_path, "metrics_fine_tuned_rebel.csv")

    with open(output_file_path, "w", newline="") as csvfile:
        fieldnames = ['Cluster', 'METEOR', 'ROUGE-1', 'ROUGE-2', 'ROUGE-L']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

        writer.writeheader()

        for cluster_key, cluster_metrics in metrics.items():
            writer.writerow({
                'Cluster': cluster_key,
                'METEOR': sum(cluster_metrics['METEOR']) / len(cluster_metrics['METEOR']),
                'ROUGE-1': sum(cluster_metrics['ROUGE-1']) / len(cluster_metrics['ROUGE-1']),
                'ROUGE-2': sum(cluster_metrics['ROUGE-2']) / len(cluster_metrics['ROUGE-2']),
                'ROUGE-L': sum(cluster_metrics['ROUGE-L']) / len(cluster_metrics['ROUGE-L'])
            })
